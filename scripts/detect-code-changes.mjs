 ...
 *
 * Excluded from code changes (don't require changesets/changelog fragments):
 * - Markdown files (*.md) in any folder
 * - .changeset/ folder (JS changeset metadata)
 * - changelog.d/ folder (Rust changelog fragments)
 ...
 *   node scripts/detect-code-changes.mjs --lang js
 *   node scripts/detect-code-changes.mjs --lang rust
 *
...
const lang = langIndex !== -1 ? args[langIndex + 1] : 'all';
...
/**
 * Filter files by language folder
 * @param {string[]} files - Array of file paths
 * @param {string} language - 'js', 'rust', or 'all'
 * @returns {string[]} Filtered files
 */
function filterByLanguage(files, language) {
  if (language === 'all') {
    return files;
  }

  const prefix = language === 'js' ? 'js/' : 'rust/';
...
function detectChanges() {
  console.log(`Detecting file changes for CI/CD (language filter: ${lang})...\n`);

  const allChangedFiles = getChangedFiles();
  const changedFiles = filterByLanguage(allChangedFiles, lang);
  ...
  const codePattern =
    lang === 'rust'
      ? /\.(rs|toml|mjs|js|yml|yaml)$|\.github\/workflows\//
      : lang === 'js'
        ? /\.(mjs|js|json|yml|yaml)$|\.github\/workflows\//
        : /\.(mjs|js|json|rs|toml|yml|yaml)$|\.github\/workflows\//;
